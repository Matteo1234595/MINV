...
def color_text(label, value, positive=True):
    try:
        val = float(value)
        color = "green" if (val > 0 if positive else val < 0) else "red"
        return f"<span style='color:{color}'><strong>{label}: {format_val(val)}</strong></span>"
    except:
        return f"<strong>{label}: {format_val(value)}</strong>"
