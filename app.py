...
if menu == "Dashboard":
    ...
    st.subheader("🤖 AI Consulente – Cosa acquistare o vendere in tempo reale")

    capitale = st.number_input("Capitale disponibile (€)", 1000, 1_000_000, 20000)
    ...
    if st.button("📊 Analizza e genera piano"):
        st.write("### ✅ Raccomandazione in tempo reale")
        suggeriti = []
        ...
        if rischio == "Basso":
            ...
            for t in titoli_growth:
                try:
                    ...
                    suggeriti.append({"Tipo": "Azione", "Ticker": t, "Motivo": "⚠️ Dati non disponibili"})
            suggeriti.append({"Tipo": "ETF", "Ticker": "DGTL.DE", "Motivo": "ETF tecnologia e digitalizzazione"})

        st.write("### 🛒 Cosa comprare:")
        for s in suggeriti:
            st.markdown(f"- **{s['Tipo']}** `{s['Ticker']}` – {s['Motivo']}")

        st.write("### 🔁 Possibile azione di vendita:")
