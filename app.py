...
menu = st.sidebar.selectbox("📊 Seleziona sezione", [
    "Dashboard", "Analisi azienda", "Portafoglio simulato", "Consulenza personalizzata", "Screener azioni italiane", "AI Consulente",
...
if menu == "Consulenza personalizzata":
    ...
    st.subheader("🧑‍💼 Consulenza dettagliata personalizzata")

    with st.form("profilo"):
        eta = st.number_input("Età", 18, 100, 45)
        capitale = st.number_input("Capitale disponibile (€)", 1000, 1000000, 25000, step=1000)
        rischio = st.selectbox("Tolleranza al rischio", ["Basso", "Medio", "Alto"])
        orizzonte = st.selectbox("Orizzonte d'investimento", ["1-3 anni", "3-5 anni", "5+ anni"])
        submit = st.form_submit_button("Analizza")

    if submit:
        st.markdown("### 📈 Strategia consigliata")
        azioni, obbligazioni, liquidità = 0.3, 0.5, 0.2
        if rischio == "Alto":
            azioni, obbligazioni, liquidità = 0.7, 0.2, 0.1
        elif rischio == "Medio":
            azioni, obbligazioni, liquidità = 0.5, 0.3, 0.2

        if orizzonte == "1-3 anni":
            azioni *= 0.7
            obbligazioni *= 1.1
        elif orizzonte == "5+ anni":
            azioni *= 1.2
            obbligazioni *= 0.9

        totale = azioni + obbligazioni + liquidità
        azioni, obbligazioni, liquidità = [x / totale for x in [azioni, obbligazioni, liquidità]]

        st.success(f"Azioni: {azioni*100:.1f}%, Obbligazioni: {obbligazioni*100:.1f}%, Liquidità: {liquidità*100:.1f}%")

        st.markdown("### 🛒 Cosa acquistare concretamente")

        if rischio == "Basso":
            st.markdown("- **ETF obbligazionario**: iShares Euro Government Bond 1-3yr (IBGL.DE)")
            st.markdown("- **Liquidità**: Conto deposito vincolato a 12 mesi (2-3%)")
        elif rischio == "Medio":
            st.markdown("- **ETF azionario globale**: Vanguard FTSE All-World (VWCE.DE)")
            st.markdown("- **ETF obbligazionario**: iShares Euro Corp Bond (IEAC.DE)")
        else:
            st.markdown("- **Azioni italiane suggerite**:")
            for ticker in ["STM.MI", "LUX.MI", "UCG.MI"]:
                try:
                    data = yf.Ticker(ticker).history(period="6mo")
                    rend = (data["Close"].iloc[-1] - data["Close"].iloc[0]) / data["Close"].iloc[0]
                    st.markdown(f"  - {ticker}: rendimento 6 mesi = {rend*100:.2f}%")
                except:
                    st.markdown(f"  - {ticker}: dati non disponibili")
            st.markdown("- **ETF tematico**: iShares Digitalisation (DGTL.DE)")

        st.markdown("### 🔍 Spiegazione dettagliata")
        st.write("In base all'età e all'orizzonte temporale, abbiamo stimato una propensione al rischio che guida la percentuale di allocazione.")
        st.write("I prodotti selezionati sono ad alta capitalizzazione e diversificazione, bilanciando crescita e protezione.")
        st.write("Azioni italiane selezionate in base alla performance a 6 mesi, ROE e dividendi.")
