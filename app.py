...
# Sezioni aggiuntive nel menu
elif menu == "Dashboard mercato":
    ...
    try:
        ...
        st.warning("⚠️ Impossibile caricare alcuni indici.")

elif menu == "Simulatore portafoglio":
    st.subheader("💼 Simulatore di Portafoglio")
    st.write("Costruisci e analizza un portafoglio simulato.")
    tickers = st.text_input("Inserisci i ticker separati da virgola (es: ENI.MI,STLA.MI)")
    peso = st.slider("Percentuale investimento per titolo (equamente distribuito)", 1, 100, 20)
    if st.button("Simula"):
        try:
            ...
            st.error("Errore nella simulazione. Controlla i ticker inseriti.")

elif menu == "Screener azioni italiane":
    st.subheader("🔎 Screener Azioni Italiane")
    st.write("Analizza e filtra le principali azioni italiane.")
    try:
        ...
        st.warning("⚠️ Errore nel caricamento titoli.")
